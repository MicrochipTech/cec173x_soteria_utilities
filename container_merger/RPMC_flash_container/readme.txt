     RPMC_flash_container Version 1.0
 ==============================================
Hash Utility - 
    > RPMC_flash_container.exe 
            -i <cfg_file_name> 


Running "RPMC_flash_container.exe" from command prompt will take 
    "rpmc_cfg.txt" as a default configuration file and 
    generators the output "rpmc_spi_image.bin"
...
Execution :
Create container:
RPMC_flash_container.exe  -i rpmc_cfg.ini -c 1
Increment container :
RPMC_flash_container.exe  -i rpmc_cfg.ini -c 5
Update RPMC contianer :
RPMC_flash_container.exe  -i rpmc_cfg.ini -c 7
Repair RPMC container:
RPMC_flash_container.exe  -i rpmc_cfg.ini -c 3
Enable unrestricted contaienr:
RPMC_flash_container.exe  -i rpmc_cfg.ini -c f
Update OTAK Key:
RPMC_flash_container.exe  -i rpmc_cfg.ini -c 1f
Modify TAGx Base Addresses
RPMC_flash_container.exe  -i rpmc_cfg.ini -c 2f

All command can be created using the command: default configuration is rpmc_cfg.ini
RPMC_flash_container.exe  

Output files: Generated the binaries under rpmc
============
contatenate_create_container_raw_to_sign.bin => create container raw file 
contatenate_create_container_signature.bin  => create container signature 
contatenate_enable_unrestricted_container_raw_to_sign.bin =>  enable_unrestricted_ raw 
contatenate_enable_unrestricted_container_signature.bin => enable_unrestricted_ signature 
contatenate_increment_container_raw_to_sign.bin  => increment container 
contatenate_increment_container_signature.bin => increment container signature 
contatenate_repair_container_raw_to_sign.bin => repair raw file
contatenate_repair_container_signature.bin => repair container 
contatenate_update_container_subcommand_0_raw_to_sign.bin => update container raw
contatenate_update_container_subcommand_0_signature.bin => update container signature 
contatenate_update_container_subcommand_1_raw_to_sign.bin => update container subcommand 1 of transfer
contatenate_update_container_subcommand_1_signature.bin => update container subcommand 1 of transfer signatuer 
contatenate_update_OTAAPub_container_raw_to_sign.bin => OTA pub raw file 
contatenate_update_OTAAPub_container_signature.bin => OTA with signature file 
modify_tagx_baseaddress_container.bin => modify raw file 
rmpc_raw.bin
rmpc_signature.bin
...
RPMC Header configuration
--------------------------------------------------------------------------------------------

; RPMC Container Header Format
; 
[RPMCContainerHeader]
RPMCValue = 0x0  ;RPMC Value [31:0] ;4 byte value  , offset 0x0, size 4
...
ContainerNumber =0x1 ; Container Number [7:0] , offset 0, size 1 byte
ContainerType =0x2 ; Container Type [7:0] , offset 1 , size 1 byte
...
OwnerConfiguration =0x10  ; Owner Configuration [7:0] , offset =0xC5 , size 1 bytes
OwnerID	=0x123456789ABCDEF123456789ABCDEF12  ; Owner ID , offset =0xC6 , size 16 bytes
KeyRevocation =0x10 ;  Key Revocation , offset 0xD6 , size 1 bytes
RollbackProtection =0x10 ; Roll back protection byte 15-0 , offset 0xD7 , size 16 bytes
TAG0ImageHeaderBaseAddress =0x2000   ; TAG0 Image Header Base Address, offset =0xE7h , size 4 bytes
TAG1ImageHeaderBaseAddress =0x900000 ; TAG1 Image Header Base Address, offset =0xEbh , size 4 bytes
; OTP Emulasion
...
OwnerDebugOptions =0x10 ; OTP Byte 347: Owner Debug Options , offset = 17Fh , size 1 bytes
OwnerPlatformID =0x1234  ; OTP Bytes 352-353: Owner Platform ID , offset = 0x180h , size =2 btes 
SecurityFeatures =0x10 ; OTP Byte 354: Security Features , offset = 0x182h , size 1 bytes
SHA384Platk = hash_of_KeyHashBlob.bin  ; OTP Bytes 864-911:: SH384(PlatK) in binary format ,offset = 0x183h, size 48 bytes 
OwnerCreationPubKey  =ECC384rPUB.pem  ;Signature Public Key (96 Bytes), offset = C5h + Sizeof (Container Type Parameters) ,  size 96 bytes
OwnerCreationPrivateKey  =ec_384_1.pem  ; Command Signature in pEM format , offset =125h + Sizeof (Container Type Parameters) , size 96 bytes
OwnerCreationPrivateKeyPass=None
...
Execution in command line option has below:

RPMC_flash_container.exe -i rpmc_cfg.txt


To convert into the bin to hex file using srec_cat:

srec_cat.exe contatenate_create_container_signature.bin -binary -output contatenate_create_container_signature.hex -intel
